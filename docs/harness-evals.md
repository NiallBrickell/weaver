...
`WorkerExecutor` seam before any of them is allowed into production selection.
...
These adapters live under `src/evals/`. They are intentionally absent from `selectExecutor`: an
eval result cannot silently change how real Workstreams execute.
...
1. `code-repair`: fix an adoption-state bug and pass hidden cases while changing one allowed file;
...
4. `image-understanding`: inspect a PNG Linear ticket whose randomized incident facts exist only in
   raster pixels, then return the exact identifier, status, environment, owner, and error in a
...
The target model must itself support image input. Kimi K3 does; a text-only model is expected to
fail this case and can still remain a candidate for text-only routes once the model router records
...
The production choice remains blocked on the confinement and supervised-action cases in
[`TODO.md`](../TODO.md), followed by managed-runtime trials.
