...
- Repo-convention injection into the coordinator — a target repo's static rulebook (CLAUDE.md/AGENTS.md) reaches workers as conventions but not the coordinator, which today sees that doctrine only through the policy store (where a full-rulebook backfill goes dormant, see Backfill above). Repo conventions should be injected into the coordinator's projection wholesale when a workstream is repo-scoped, keeping the policy store for genuinely *learned* corrections rather than a mirror of a rulebook.
