...
$W steer $SLUG "Company facts pack, answering your open blockers. Company: Loomworks. Product: an API that turns messy operational spreadsheets into governed internal tools. Stage: pre-seed, \$1.2M raised from Basecamp Ventures, 14 months runway. Team: two founders — Priya (CEO, ex-Stripe product) and Marco (CTO, ex-Datadog infra; he is technical and currently writes all the code). Stack: TypeScript, Node, Postgres, React, deployed on Fly.io. Remote policy: remote-first in EU/UK timezones, quarterly onsites in Lisbon. Compensation: EUR 90-115k plus 1.0-2.0% equity, no bonus. Interview loop: intro call, 2h paid working session on real code, founder chat, offer within a week. First candidate to contact: Sam Alvarez <sam@example-candidate.dev> — I read their open-source spreadsheet-diff library and the merge-conflict UI in particular is exactly the taste we want; mention that specifically. Sign outreach as Priya, CEO. If you flagged any role-shape claims for confirmation: confirm them all as true of us as written. Fill every placeholder from this message only — anything I have not stated here gets its sentence cut, not approximated. Treat the JD as publishable once filled, and proceed to outreach — my approval is still required before anything is sent."
