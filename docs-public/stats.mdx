---
title: "The convergence dashboard"
description: "Is the human getting out of the loop? One page, computed from durable state, that answers it over time"
---

`yarn weaver stats` renders a single self-contained HTML page of fleet-wide metrics — no server, no dependencies, safe to keep open in a tab and regenerate whenever you want a fresh read.

The page exists to answer one question over time: **is the number of human interventions per successful outcome falling?** That ratio is the system's explicit optimization target, guarded by two things that must *not* fall with it — output quality (rejections stay visible) and the authority firewall (autonomy over judgment never becomes autonomy over authority).
