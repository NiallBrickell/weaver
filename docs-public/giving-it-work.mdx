...
3. **Change the world through Pilot.** A worktree, pull request, send, or API call is a separate action. [Pilot](https://github.com/erdoai/pilot) approves routine-safe commands inside the live run; anything with real blast radius arrives on your [needs-you queue](/dashboard). Your constraints remain hard ceilings.
