...
- **SDK estimate per successful outcome** — total SDK-reported estimate over qualified conclusions, with the estimate per adopted work product kept beside it as a leading indicator. It is diagnostic telemetry, not provider billing.
...
- **Per workstream** — whether the stream reached a qualified conclusion, plus passes, adoptions, rejections, interventions, auto-approval ratio, and SDK estimate. The fleet trend is the signal; this table is where to look when it moves.
