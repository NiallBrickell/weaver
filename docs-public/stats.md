...
- **Per workstream** — whether the stream reached a qualified conclusion, plus passes, adoptions, rejections, interventions, and auto-approval ratio. The fleet trend is the signal; this table is where to look when it moves.
...
- **Undated interventions are reported, not hidden.** A few legacy/config edits incremented the lifetime counter without leaving a durable timestamp; the dashboard shows them as an explicit remainder instead of silently dropping them from the timeline.
- **One keypress counts once, and only human keypresses count.** An approval that auto-resolves its attention card is one act, not two. Resolutions stamped by a system/harness actor — the pilot daemon (`pilot`), the coordinator (`coordinator`), the worker (`worker`), fleet capacity bookkeeping (`fleet-capacity`, `capacity-probe`), the one-time execution-safety migration (`execution-safety-migration`), or any engine-driven act (an actor name starting with `engine:`, e.g. a readback closure or a capacity-recovered closure) — are never interventions, and legacy records that predate actor attribution fall into the undated remainder rather than being guessed at.
