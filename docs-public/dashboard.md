...
**Cards close themselves when the world settles them.** When a workstream raises a card that only matters until something outside Weaver happens — "merge or close PR #2686?", "is Sentry issue GO-ZX still firing?" — it declares that fact on the card, and the runner reads it back: the card closes on its own once the PR is MERGED/CLOSED or the issue is resolved/ignored, records what it saw (state, time, link), and wakes the workstream to carry on. PR state is read with the fleet's GitHub App read token; Sentry needs an optional read-only token (`weaver secret set WEAVER_SENTRY_READ_TOKEN --executor`). With neither configured, or while the provider cannot be reached, the card simply stays open. A card that merely *mentions* a PR is never closed on that basis — the question may still be live. Capacity notices clear the same way: once their provider works again, or the workstream's work is flowing on another model, they are gone without you. None of these count as your interventions in [stats](./stats.md). For cards raised before this existed, `weaver attention-hints` lists the ones citing a PR that has since merged or closed; `weaver attention-hints --apply` hands each workstream one note so it can withdraw the card itself if the PR was all it was waiting on.
...
The fleet is a board of Workstreams, not agent sessions. Its four lanes are **Needs you**, **In motion**, **Waiting**, and **Ready**. Each card says **Now** in plain language, then shows the current **Course** and any recent direction from you. It does not make you decode storage ids or three overlapping counters. Search narrows the board; filters isolate recent movement and routines; long lanes are bounded rather than becoming an endless wall. Concluded Workstreams remain available under the folded **Done** list.

Inside a Workstream, the board changes level: cards are Assignments in **Planned**, **Working**, **Review**, and **Accepted**. A worker offers a **Proposed result**; the card enters **Accepted work** only when the Assignment is completed and the coordinator accepts that result. Attempts stay nested under their Assignment as execution history; a finished attempt cannot turn proposed work green. Failed, cancelled, rejected, superseded, and completed-but-unaccepted work stays inspectable in the archive.

Your newest direction sits above the current recorded commitment with an exact time and an honest state: **Waiting for Weaver**, **Read by Weaver**, or **Withdrawn before Weaver read it**. “Read” does not overclaim that a particular decision applied it. Course decisions have their own timestamps, and a full typed direction history remains below. Storage ids and execution provenance recede behind **Technical details**; PR numbers, branch names, and other recognizable external references remain visible. Waits, accepted evidence, action readbacks, interactions, and relevant learned policies stay on the Workstream page without competing with current work. The full policy store remains a separate **Learned** page.

Each Workstream page names the state revision and the time its static snapshot was generated. An already-open tab does not silently pretend to be live: run `weaver inspect` or press `i` again to regenerate it. Everything rendered is a projection of typed state—no transcript parsing and no idle-timer liveness guesses. A workstream whose state cannot be read renders as a loud failure, never an empty screen.
