...
| Capability is not authority | Every assignment has the normal Code toolset. `submit_result` is the only way a worker proposes Workstream state; intentional external effects still use a separate action lifecycle |
