...
| Accepted inputs gate dependent work | A dependent assignment runs only after every prerequisite is completed and accepted. Weaver refuses a new dependency already settled without acceptance and raises one durable blocker if an existing prerequisite later becomes missing, failed, cancelled, rejected, or superseded, so impossible work cannot sit silently queued forever |
...
| Infrastructure outages cost waiting, not strikes | A coordinator or worker hitting plan/session/rate limits, overload, or auth failure writes executor/provider/model-scoped `WorkstreamDoc.capacity` state and parks only that target behind a typed backoff wake — for the whole fleet, since every other workstream borrows the active wait instead of relaunching into it, until any later success releases it or one probe re-tests it on expiry; workers keep moving and the coordinator uses one configured fallback while available. The dashboard says `WAITING` only when this blocks the next intended transition. Auth surfaces one recovery card immediately; other capacity failures get 12 backoffs to self-clear. Weaver retries at the provider reset, probes only local Claude SDK targets on Claude credential-metadata change, or makes the wait due after explicit `weaver capacity retry`. Fresh provider-reported headroom may appear in the fleet header; missing telemetry stays unknown. [Recovery stays single-principal](./claude-capacity.md): Weaver never changes billing, handles tokens, or cycles accounts |
