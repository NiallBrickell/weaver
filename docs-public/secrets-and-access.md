...
Weaver does not add a second process sandbox in this MVP; the environment that launches the local executor owns containment. Capability still does not grant action authority. A non-action brief is not permission to push, open or merge a PR, deploy, send, or mutate a remote service. The coordinator directs those effects through a typed action, where Pilot supervises the live calls, Weaver secrets are injected only for the approved run, and deterministic readback decides whether the effect happened.
