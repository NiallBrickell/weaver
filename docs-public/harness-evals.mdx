...
description: "How Weaver compares disposable agent runtimes without weakening its durable contracts"
---

Weaver owns the durable Workstream, not the agent loop underneath it. Before replacing the current
Claude Agent SDK worker, Weaver runs candidate runtimes through the same real assignment,
submission, and adoption boundary.
...
<Warning>
  A local task-quality result is not proof of production containment. Host-process candidates rely
  on the environment that launched Weaver, and a local OpenHands container is not a managed
