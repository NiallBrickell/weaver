...
| Infrastructure outages cost waiting, not strikes | A coordinator or worker hitting Agent SDK credits, rate limits, overload, or auth failure writes model-indexed `WorkstreamDoc.capacity` state and parks the stream behind a typed backoff wake; queued model work stops instead of becoming a retry storm. Credit/auth failures surface one recovery card after 3 consecutive backoffs, session/rate limits after 12. The runner probes each affected model and clears matching state fleet-wide on recovery. [Recovery stays single-principal](/claude-capacity): Weaver never handles tokens or cycles accounts |
