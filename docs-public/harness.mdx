...
| Infrastructure outages cost waiting, not strikes | A coordinator or worker hitting Agent SDK credits, rate limits, overload, or auth failure parks the stream behind a durable typed backoff wake; queued model work stops instead of becoming a retry storm. The runner probes each affected model when credentials change (and periodically otherwise), then expedites matching backoffs fleet-wide when capacity returns. [Recovery stays single-principal](/claude-capacity): Weaver never handles tokens or cycles accounts |
