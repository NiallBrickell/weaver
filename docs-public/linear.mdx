...
A coordinator pass can only look. Retrieval calls work; anything that would
change Linear is refused outright, because a coordinator is the seat that
*decides*, and a change it made itself would be a change nobody approved.
Posting a comment or moving an issue to Done is therefore a real-world action:
it waits for your approval, a worker performs it, and it counts as done only
once Weaver reads Linear back and confirms it landed.
