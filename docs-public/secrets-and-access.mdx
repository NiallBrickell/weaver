...
Everything rides the local Claude Code subscription login. `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, and `CLAUDE_CODE_OAUTH_TOKEN` are stripped from every spawned session, so a stray exported credential cannot silently change the principal or switch billing to API credits. The dollar figures on the dashboard are SDK-reported estimates used only as a runaway backstop.
