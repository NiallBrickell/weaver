...
Research workers get read access only: directory grants, plus the same operator MCP servers behind a deterministic read-only gate — retrieval calls (`get`/`list`/`search`/`query`/…) work, anything that would mutate external state is denied at the tool boundary, no Bash, no credentials. Querying your error tracker is research; changing it is an action. Side effects are what approval unlocks.
