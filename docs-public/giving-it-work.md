...
A bad name is never permanent. `weaver rename <slug> <new-slug>` moves the whole identity to a better name: history, revisions, artifacts and their pinned hashes, manager links from other workstreams, and the learning layer's policy attribution all follow, and the old name stays visible in the stream's event log as lineage. The rename is refused while the stream is mid-tick (in-flight work still answers to the old name — retry a few seconds later) and when the target name is taken.
