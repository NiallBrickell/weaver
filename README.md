...
Weaver keeps the durable outcome above interchangeable disposable model loops. Local Claude (the default), local Codex, and the OpenHands container can supply agents that enter the work, advance it, and leave:
...
- **Operator workspace**: `weaver ui` opens a browser surface for creating work, seeing the fleet board, opening one Workstream, and adding follow-up. The UI serves state; a separate `weaver run` process executes work. Follow-up is recorded as an untrusted Observation, so browser access never silently becomes authority. See [docs-public/operator-workspace.md](./docs-public/operator-workspace.md).
- **Visual work board**: `i` in the dashboard, or `weaver inspect [slug]`, opens a self-contained read-only browser view. The fleet groups Workstreams into Needs you, In motion, Waiting, and Ready; every card says what is happening now, the current course, and any recent direction from you without exposing storage ids or record counters. A Workstream's page puts timestamped human direction above its recorded commitment, then groups Assignments into Planned, Working, Review, and Accepted work. Each page names its snapshot revision and generation time. Attempts remain nested technical provenance, and regenerating the view never pretends it was read; exact catch-up belongs to the delivered printout checkpoint. See [docs-public/dashboard.md](./docs-public/dashboard.md).
- **Printouts**: uppercase `P` in the interactive dashboard follows the selection (`W E A V E R` header = all workstreams + global policy activity), writes an immutable self-contained engineering document, and opens it in the browser; lowercase `p` remains pause. It is one narrow, long-form reading page: every selected workstream and its exact organizational timeline appears directly in normal document flow, with nothing hidden behind cards or disclosure controls. The visual board (`i`) links to a Catch up hub containing every published window, and each page can copy its complete plain-text report with its button or uppercase `C`. Exact typed before/after sidecars preserve eventless and intermediate changes without bumping organizational revisions; SDK dollar-estimate telemetry stays out of the operator report. Tool calls can show what a worker looked at, but only typed adoption and deterministic readback prove an external outcome. See [docs-public/printouts.md](./docs-public/printouts.md).
- **One fleet, many machines**: `WEAVER_STORE=postgres://…` points every machine at the same knowledge store (any plain Postgres — Supabase, Neon, RDS, self-hosted): the laptop CLI, the dashboard, and remote runners then share the same decisions, learned policies, and needs-you queue. Reviewed routes change models only inside the configured worker substrate; heterogeneous runners separately declare what each host can launch with `WEAVER_RUNNER_EXECUTORS`, and the attempt/lease claim refuses unsupported work before state moves. For a single machine, `WEAVER_STORE=sqlite:<path>` keeps the same transactional contract in one local file (Node's built-in `node:sqlite`, zero dependencies). The default remains local fs state under `./state`. See [docs-public/hosted-state.md](./docs-public/hosted-state.md).
...
- **Secrets**: `weaver secret set NAME [--ws slug]` reads action credentials from stdin; models see names only, approved action shells get values, and every captured result is scrubbed. Provider keys use the stricter `--executor` scope: neither name nor value reaches a model or shell. The store refuses any write embedding either kind of known value.
- **Operator access**: every assignment is a regular coding-agent worker with shell, file editing, web tools, and the operator's configured MCP servers used read AND write where its executor supports them. A worker has exactly two lifecycles: `work` (bounded, reversible work that proposes a result) and `action` (one approved, Pilot-supervised, readback-verified irreversible egress). Keeping the systems a brief names in sync over those MCP servers — moving a tracker issue's status, commenting, labelling — is ordinary `work`, no approval or allow-list. Workers run with a 200-turn ceiling by default. Typed work requirements may select a reviewed eval-proven model route; the selected target is pinned on the attempt, never on intended work (see [Model routing](./docs-public/model-routing.md)). Actions never enter that router and default to local Claude because Codex, Pi, and OpenHands cannot yet expose Pilot's live callback. [Where model loops run](./docs-public/executors.md) covers the executor contracts and configuration.
- Auth rides one ambient operator principal: Claude Code login for `local-sdk`, ChatGPT login for `codex-sdk`. Exported provider API credentials are stripped from those subscription-backed subprocesses, and Weaver never stores tokens, pools credentials, or cycles accounts. Capacity failures park only the affected executor/provider/model; the coordinator uses its configured fallback when available. See [Claude capacity & billing](./docs-public/claude-capacity.md) and [Execution safety](./docs-public/execution-safety.md).
