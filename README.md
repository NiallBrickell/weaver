...
- **Visual work board**: `i` in the dashboard, or `weaver inspect [slug]`, opens a self-contained read-only browser view. The fleet groups Workstreams into Needs you, In motion, Waiting, and Ready; a Workstream's page groups its Assignments into Planned, Working, Review, and Accepted. Accepted means completed **and adopted**, while attempts remain nested execution history. Search, routine/recent filters, bounded lanes, and a folded Done list keep a large fleet scan-friendly. Regenerating this live view never pretends it was read; exact catch-up belongs to the delivered printout checkpoint. See [docs-public/dashboard.md](./docs-public/dashboard.md).
