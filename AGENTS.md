...
- [`src/coordinator.ts`](./src/coordinator.ts) — fresh `query()` per pass; mutation tools are the only write path; `tools: []`, `permissionMode: 'dontAsk'`, `settingSources: []`, `strictMcpConfig: true`, `persistSession: false` are load-bearing, not defaults to tidy up.
- [`src/worker.ts`](./src/worker.ts) — fresh run per assignment; `submit_result` is the worker's entire write surface. The model loop runs behind the `WorkerExecutor` seam ([`src/executor/`](./src/executor/), `WEAVER_EXECUTOR`, default local SDK `query()`); supervision and submit callbacks stay harness-owned.
