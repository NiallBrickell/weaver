...
- **The daily digest is operator notification: operator-configured destination, model-free content, readback-idempotent.** `weaver digest --post` renders typed state to a destination fixed in the executor-only store, and a post with an unknown result is read back, never re-sent. Posting anything to a team channel or another person remains an action. See [docs/harness.md](./docs/harness.md).
...
- **A card that exists until an external fact holds declares the fact (`resolves_when`) and the runner reads it back; prose mentioning a PR never auto-binds a card.** The readback closes a card only on an exact provider answer (host-side tokens, fail open), stamps an `engine:*` actor that never counts as a human intervention, and wakes an active stream. Capacity cards close from typed state once their target or their role's work flows again. Legacy prose gets a hint Observation for the coordinator to judge, never a close. See [docs/harness.md](./docs/harness.md#how-needs-you-cards-close).
- **Never put a personal GitHub login or PAT on a hosted runner.** Hosted repository access uses a dedicated GitHub App installed across the intended repository estate (all organization repositories for an organization-wide fleet, or a selected subset for a narrower fleet); every short-lived token is still narrowed to the exact repository and read/write lifecycle. Ordinary containers get neither the App key nor a token, same-UID model actions are refused, and only an exact approved engine command receives write capability. Ordinary workers also cannot mount the state or secrets directories (a worker directory that is, contains, or sits under one is refused at creation and again before launch), and engine-run commands never inherit `WEAVER_STORE` or model credentials. See [docs/harness.md](./docs/harness.md).
...
- **Never loop `load()` over the fleet on a hot path.** On a hosted store every document body a runner reads crosses the billed public proxy; the runner poll (2026-08-29) and then `listManagedBy()` at every coordinator pass (2026-09-08) each turned that into the dominant Railway charge and the second one stopped the whole account. Anything that needs one field of every workstream is a head column and a `StateStore` method (`listWorkstreamHeads`, `listManagedBy`, `findBySourceKey`). See [docs/harness.md](./docs/harness.md).
- **A seat parked for one workstream is parked for the fleet.** An active capacity wait on an exact executor/provider/model target is shared: a stream about to launch there borrows it (`observedIn`, no backoff counted, no attention) instead of spending a launch to rediscover the limit — 96.4% of 5,411 coordinator backoffs over 30 days started under another stream's active wait on the same seat. A later success anywhere refutes it, an expired unrefuted wait admits ONE probe launch, and `other`/`auth` waits stay per host. See [docs/harness.md](./docs/harness.md).
- **A probe is shell the engine runs repeatedly, so it is gated like an action — approval pinned to the exact spec, a minimal env that never inherits process.env, read-only credentials only — and an unchanged check never writes the workstream doc.** Its checking state lives in the narrow `probe_cursors` table, because a doc write per check re-transfers every body to every runner. See [docs/harness.md](./docs/harness.md#probes-engine-run-checks-gated-like-actions).
- **A fresh heartbeat is not health — a runner that cannot COMMIT publishes itself degraded and dispatches nothing.** The store writes a printout receipt under `WEAVER_HOME` before every COMMIT, so a full disk (2026-09-10: a 30 GB VM, filled by worker leftovers) failed every tick before any write for four days while the DB-only presence beat every five seconds. `runLoop` probes the state directory each poll and, while it fails, publishes `RunnerPresence.degraded` (no seats, reason shown by status/watch) and launches no tick. Any new "is the runner ok" signal must be derived from the ability to commit, never from liveness alone. `/healthz/fleet` also reports unhealthy when due work goes unserved for an hour, or no pass completes for 12h while work waits on capacity, so any new health signal must be derived from committed output. See [docs/harness.md](./docs/harness.md).
- **A step is progress, not a decision — `record_progress` updates the standing course in place; supersede only when the commitment changes.** Used as a step log, decisions minted 1,635 records in 30 days (73% superseded, median life 35 minutes) and put ~2k-character rationales into every projection. Progress cites typed facts and never grants authority. See [docs/harness.md](./docs/harness.md#decisions-are-commitments-progress-is-position).
...
- **The human's standing self-merge bar (2026-08-04) is merge-commit only.** Workstreams merging their own DevBot-clean, CI-green PRs in repos covered by the grant use `gh pr merge <n> --merge` — never squash or rebase, which the repo settings reject. The grant itself is the operator's own Pilot rule — a learned fleet policy can never grant authority (kernel rule 11) — so the hosted Pilot must carry the same self-merge carve-out (`bin/weaver-gcp.sh push-pilot-config`); this line records its merge method and that no-merge rules elsewhere are scoped to humans' assistants, not to that grant.
