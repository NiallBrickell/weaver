...
- **The founder's standing self-merge bar (2026-08-04) is merge-commit only.** Workstreams merging their own DevBot-clean, CI-green PRs in repos covered by the grant use `gh pr merge <n> --merge` — never squash or rebase, which the repo settings reject. The grant itself lives in the fleet's authority policy with its provenance; this line records its merge method and that no-merge rules elsewhere are scoped to humans' assistants, not to that grant.
