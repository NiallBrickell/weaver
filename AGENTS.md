...
1. **Four identities stay distinct.** *Workstream* (months–years: outcome, constraints, current state, standing decisions, execution/resource policy, waits) · *Agent* (a reusable worker definition) · *Assignment* (one bounded responsibility with acceptance criteria and attempt history) · *Run* (one execution attempt, minutes–hours). A run is never intended work; treating a run as a task loses the work when the worker is replaced.
...
- **Never let a repo egress (`gh pr create`/`gh pr merge`/`git push`) fire into a collision.** A colliding open PR is a conflicting arrival on shared external state (invariant 8 across the git seam): the deconfliction gate holds the action for the human — reconcile, don't re-write. It fails OPEN on tooling failure but CLOSED on a detected collision. See [docs/harness.md](./docs/harness.md).
...
- [`src/coordinator.ts`](./src/coordinator.ts) — fresh `query()` per pass; mutation tools are the only write path; `tools: []`, `permissionMode: 'dontAsk'`, `settingSources: []`, `strictMcpConfig: true`, `persistSession: false` are load-bearing, not defaults to tidy up. **Keep it plain** — the coordinator has no tools onto the outside world and must not grow any; anything it needs to see is a worker assignment it dispatches and adopts. `create_workstream` carries `source_key` (idempotent on `WorkstreamCore.sourceKey`) so intake is a workstream rather than a subsystem — see [docs/intake.md](./docs/intake.md).
- [`src/worker.ts`](./src/worker.ts) — fresh regular Code run per assignment; `submit_result` is its only Weaver submission API, while the launching substrate owns process containment. The model loop runs behind the `WorkerExecutor` seam ([`src/executor/`](./src/executor/), `WEAVER_EXECUTOR`, default local SDK `query()`); action supervision and submission callbacks stay harness-owned.
...
- **Erdo-org self-merge grant (founder, 2026-08-04; merge method corrected 2026-08-14).** A workstream MAY merge a PR it authored on an erdoai org repo when BOTH hold: DevBot's latest completed review at the exact head SHA reports zero concrete issues, AND every CI check is green. The merge is always a regular merge commit (`gh pr merge <n> --merge --repo erdoai/<repo>`) — squash and rebase merges are disabled at repo settings and must never be attempted — and the result is confirmed by readback. Outside erdoai repos, for PRs the workstream did not author, or when either condition is unmet, merging stays a human decision.
