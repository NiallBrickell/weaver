...
- **The erdoai self-merge bar is the founder's standing grant (2026-08-04), and it is merge-commit only.** Workstreams merging their own DevBot-clean, CI-green erdoai PRs use `gh pr merge <n> --merge` — never squash or rebase, which the repo settings reject. The grant itself lives in the fleet's authority policy with its provenance; this line records its merge method and that no-merge rules elsewhere are scoped to humans' assistants, not to that grant.
