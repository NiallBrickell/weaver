...
7. **Capability is not authority.** Every assignment gets the ordinary coding-agent surface; `research` is a work label, not a weaker runtime. A non-action brief does not authorize intentional egress. A drafted message is a work product; sending it is a separate action with authority revalidated immediately before egress. The provider result is an external fact to reconcile. **An unknown result triggers readback, never a second send.**
...
- [`src/coordinator.ts`](./src/coordinator.ts) — fresh `query()` per pass; mutation tools are the only write path; `tools: []`, `permissionMode: 'default'` + the fail-closed `canUseTool` gate, `settingSources: []`, `strictMcpConfig: true`, `persistSession: false` are load-bearing, not defaults to tidy up. It also gets the operator's MCP servers READ-ONLY, so a coordinator can check the outside world for itself, and `create_workstream` carries `source_key` (idempotent on `WorkstreamCore.sourceKey`) so intake is a workstream rather than a subsystem — see [docs/intake.md](./docs/intake.md).
