...
test('set/list/rm roundtrip; workstream overlay wins over global; file is 0600', async () => {
  await makeWs('sec-ws');
...
test('invalid names and empty values are refused', async () => {
  assert.throws(() => setSecret('lower_case', 'x-value'));
  assert.throws(() => setSecret('GH TOKEN', 'x-value'));
  assert.throws(() => setSecret('OK_NAME', ''));
});

test('redactSecrets scrubs every value, longest first, and skips tiny values', async () => {
