...
export const HOUSE_CONSTRAINTS = [
  'Research first; every worker has the normal Claude Code toolset. Repository investigation and implementation happen in a fresh git worktree under /Users/niall/work/erdo/workspace (branch from origin/main) — never in the user\'s checkouts. Opening or merging a PR, deploying, sending, or mutating a remote service remains a gated action',
  'Heavy commands (encore test, go build ./..., yarn build:check) must run through /Users/niall/work/erdo/erdo-development/scripts/with-heavy-lock.sh; prefer targeted package tests over full sweeps',
 ...
 * ~/work/erdo with a regular worker, but a named repo skips that pass.
 * Maintained by hand; keep entries to what a task message might mean.
 */
export const REPO_MAP = `Known repos under /Users/niall/work/erdo (supply the parent dir as worker context to search across them):
- erdo — the main product: Encore Go backend (backend/), Next.js frontend (frontend/), e2e tests, evals. Default guess for product features, uploads, threads, approvals, integrations, accounts.
- maurice — leads/growth engine: lead capture, enrichment, brand-style skills, growth experiments, landing pages.
- devbot — the PR-review bot (deployed on Railway, api devbot.erdo.ai).
- engineering — the engineering.erdo.ai public site (blog, docs pages).
- erdo-data-platform — EDP: data syncs and pipelines.
- erdo-development — dev tooling and scripts (worktree setup, heavy-lock).
- pilot — the local command-approval daemon.
- weaver — this harness itself.
- erdo-cli, erdo-ts-sdk, erdo-python-sdk, erdo-integrations, codeexec — CLI, SDKs, integrations, code-execution service.`;
...
export async function onboard(message: string, done?: string): Promise<Derived> {
  ...
  await createWorkstream({
    ...
    tags: d.routine ? ['erdo', 'routine'] : ['erdo'],
