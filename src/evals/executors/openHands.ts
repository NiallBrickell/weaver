...
export interface OpenHandsEvalOptions {
  apiKey?: string;
  baseUrl?: string;
...
export class OpenHandsEvalExecutor implements EvalExecutor {
  ...
  constructor(options: OpenHandsEvalOptions = {}) {
    this.apiKey =
      options.apiKey ??
  ...
  private validateRequest(req: WorkerExecutionRequest): void {
    if (req.supervise) {
      throw new UnsupportedOpenHandsRequest(
        'OpenHands eval executor does not support action-worker supervision',
      );
    }
    if (!this.apiKey) {
      throw new UnsupportedOpenHandsRequest(
        'OpenHands eval requires WEAVER_OPENHANDS_API_KEY (or LLM_API_KEY)',
      );
    }
    ...
    if (extraDirectories.size > 0) {
      throw new UnsupportedOpenHandsRequest(
        'OpenHands eval mounts only cwd; distinct additionalDirectories are unsupported',
      );
    }
  }
...
function abortError(): Error {
  const error = new Error('OpenHands eval aborted');
