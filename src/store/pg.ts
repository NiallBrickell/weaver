...
import { emptyPolicyStore, eventHelperFor, initialDoc } from './doc.js';
...
export class PgStore implements StateStore {
  ...
  async mutate(slug: string, expectedRevision: number, fn: Mutator): Promise<WorkstreamDoc> {
    ...
    try {
      ...
      const r = await client.query('SELECT doc, revision FROM workstreams WHERE slug = $1', [slug]);
      if (r.rowCount === 0) throw new Error(`no workstream '${slug}' in the Postgres store`);
      const stored = r.rows[0].revision as number;
      if (stored !== expectedRevision) throw new RevisionConflictError(expectedRevision, stored);
      ...
      if (upd.rowCount === 0) {
        ...
        throw new RevisionConflictError(expectedRevision, (actual.rows[0]?.revision as number) ?? -1);
      }
