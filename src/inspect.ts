...
function policyRow(p: PolicyRecord): string {
  ...
  const contested = p.contested
    ? `<p class="meta"><span class="warn">contested — under review</span> ${esc(p.contested.note)} (${contestSource})</p>`
    : '';
...
<summary><span class="pill status-${p.status}">${p.status}</span><span class="row-title">${esc(firstLine(p.statement, 150))}</span>${doctrine}${p.contested ? '<span class="pill warn">contested</span>' : ''}<span class="pill effect">${esc(p.effect.kind)}</span>${evidenceTally(p)}</summary>
<div class="row-body">
 ...
 */
/**
 * The learned page: what the fleet has learned, grouped by how much the fleet
 * actually knows about it. Every policy lands in exactly one group.
 *
 ...
 * full rows there, since a rule the fleet has argued with is worth the
...
export function renderLearnedHtml(policies: PolicyRecord[]): string {
  ...
  if (!policies.length) {
    ...
    const groups = [
      doctrine.length
        ? `<h3>Doctrine — your own rules (${doctrine.length})</h3><p class="hint">Taken from the rules files you maintain and the directives you gave. These bind without evidence and outrank learned policies where their scopes overlap; re-running <code>weaver backfill</code> refreshes them from the current file.</p>${doctrineContested.length ? rows(doctrineContested) : ''}${doctrineRest.length ? `<details><summary>All doctrine (${doctrineRest.length})</summary>${rows(doctrineRest)}</details>` : ''}`
        : '',
