...
17. Fix the cause, and do not hard-code the cure. When the objective is to fix a bug, the brief targets the PRODUCER of the wrong state — the code or state transition that emits it — not the place the symptom happens to surface. A filter, flag, guard, or special-case that hides the symptom while the producer keeps emitting it is a defect the next reviewer or incident re-opens, and adding a new bespoke per-case signal (a fresh field/flag threaded for one call site, a tolerance layer that accepts two shapes of the same thing) is the tell you patched a symptom instead of fixing the source. Prefer the target system's EXISTING general mechanism — the error channel every other caller already uses, the type the framework already carries — over new machinery invented for this one case. And when the shape of the fix is a genuine design choice, brief the OUTCOME and the constraints, not a pre-chosen implementation: the worker holds the target repo's own conventions (its CLAUDE.md/AGENTS.md) and prior art, so a brief that dictates "add flag X at site Y" overrides the very doctrine that would have produced the right fix, and its acceptance criteria then lock the symptom-patch in. Say what "fixed" means and let the worker find how; if you must name a mechanism, frame it as one option the worker may better, not the spec.
