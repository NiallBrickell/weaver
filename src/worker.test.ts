...
test('the fleet steward receives only fresh narrow attention evidence in its neutral workspace', async () => {
  ...
  try {
    ...
    assert.match(request.prompt, /open human asks plus their current typed reference state, approval-service waits, active capacity backoffs, unhealthy routine signals/);
    ...
    assert.match(request.prompt, /Account for EVERY supplied ask and health signal: every humanNeeds entry, approvalServiceWait, activeCapacityBackoff, unhealthy routineHealth field, incident, and unreadable Workstream/);
    assert.match(request.prompt, /compact coverage ledger keyed by source slug \+ revision \+ entity id/);
    assert.match(request.prompt, /Group entries by their shared causal failure/);
