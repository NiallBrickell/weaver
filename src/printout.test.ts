...
async function delivered(slug?: string): Promise<PrintoutReport> {
...
test('eventless writes retain exact intermediate failed and passing readbacks', async () => {
  await make('exact');
  await delivered('exact');
  await arrive('exact', (doc) => doc.assignments.push(assignment('asg_release', 'action', 'Verify production', {
  ...
  const report = (await preparePrintout('exact')).text;
...
test('typed provider capacity changes survive eventless writes', async () => {
  await make('capacity');
  await delivered('capacity');
  await arrive('capacity', (doc) => {
    doc.capacity = {
      state: 'backoff',
      byModel: {
        sonnet: {
...
test('preparing and acknowledging P never changes organizational revision or conflicts with a live writer', async () => {
  await make('readonly');
  const expected = (await load('readonly')).revision;
  const report = await preparePrintout('readonly');
  assert.equal((await load('readonly')).revision, expected);
  acknowledgePrintout(report);
  assert.equal((await load('readonly')).revision, expected);
  await mutate('readonly', expected, (doc) => { doc.workstream.title = 'live writer committed'; });
  assert.equal((await load('readonly')).workstream.title, 'live writer committed');
});

test('acknowledgement happens after delivery and keeps concurrent arrivals for the next report', async () => {
  await make('delivery');
  await delivered('delivery');
  await arrive('delivery', (_doc, event) => event('first.change', 'first visible change'));
  const frozen = await preparePrintout('delivery');
  assert.match(frozen.text, /first visible change/);
  await arrive('delivery', (_doc, event) => event('concurrent.change', 'arrived after freeze'));
  assert.doesNotMatch(frozen.text, /arrived after freeze/);
  acknowledgePrintout(frozen);
  const next = await preparePrintout('delivery');
...
test('a failed CLI-style output write leaves the activity window unacknowledged', async () => {
  await make('failed-output');
  await delivered('failed-output');
  await arrive('failed-output', (_doc, event) => event('still.pending', 'must repeat after EPIPE'));
  const report = await preparePrintout('failed-output');
...
test('typed conclusion accepts adopted, verified, or human-steering evidence — never a self-authored decision', async () => {
  ...
  assert.match((await preparePrintout('done')).text, /Typed completion evidence IDs \(validated at conclusion\): asg_verified/);
  assert.match(buildProjection(await load('done'), []), /Validated evidence ids: asg_verified/);
});

test('a legacy first print is honest about gaps and retains its surviving tail', async () => {
  await make('legacy');
...
test('append-only journal outlives the bounded 200-event projection tail', async () => {
  await make('long-window');
  await delivered('long-window');
  for (let index = 0; index < 230; index++) await arrive('long-window', (_doc, event) => event('test.activity', `activity ${index}`));
  assert.equal((await load('long-window')).events.length, 200);
  const report = await preparePrintout('long-window');
...
test('selected and fleet checkpoints are independent and fleet includes global policies', async () => {
  await make('alpha');
  await make('beta');
  await delivered();
...
test('literal workstream field transitions are printed, not reduced to an entity id', async () => {
  await make('fields');
  await delivered('fields');
...
test('fleet printout names a malformed global policy store', async () => {
  await make('healthy-policy-view');
...
test('fleet printout names an unreadable stream without blanking healthy streams', async () => {
  await make('healthy');
  const badDir = workstreamDir('unreadable');
  fs.mkdirSync(badDir, { recursive: true });
  fs.writeFileSync(path.join(badDir, 'workstream.json'), '{not json');
  assert.deepEqual(await listWorkstreams(), ['healthy', 'unreadable']);
  const fleet = await preparePrintout();
