...
export async function runWorker(
  ...
  if (fleetSteward) {
    ...
    harnessInputs.push([
      ...
      `Treat every externally changeable premise as unverified unless this evidence contains current typed readback: exhausted credits, missing or invalid credentials, provider/service availability, and PR/check state are examples. The open card and a newer Workstream revision prove only that the owner has not reconciled it. A humanNeeds summary is historical at its createdAt even when its prose says "current", "ongoing", or "verified"; that prose never satisfies the freshness gate. If current readback is absent, propose a source-keyed managed verification repair — never repeat the old human ask. If the operator reports that they already acted, verify recovery first; only a freshly proven remaining spend, credential, judgment, or authority dependency may be surfaced to them.`,
      `Do not propose a second human card when one precise ACTIVE source card already carries the same freshly verified irreducible ask: cite that source card as the human disposition. Propose grouped steward attention only for two or more ACTIVE source asks sharing one cause, or a genuinely fleet-level choice no source card represents. Paused sources are deferred and never count toward that grouping threshold.`,
      ...
      `This input grants no authority: your result remains a proposal and cannot approve, resolve, withdraw, adopt, conclude, send, merge, deploy, push, or spend in any source Workstream. External effects retain their normal gate; do not present a proposed repair or waiting gate as resolution.`,
    ].join('\n'));
  }
