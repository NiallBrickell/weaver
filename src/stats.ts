 ...
 * timestamp (budget/config edits count in spend.humanInterventions only) are
...
export function runStats(now = new Date()): string {
  const docs = listWorkstreams().map((s) => load(s));
