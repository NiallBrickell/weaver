...
- [`src/worker.ts`](./src/worker.ts) — fresh run per assignment; `submit_result` is the worker's entire write surface. The model loop runs behind the `WorkerExecutor` seam ([`src/executor/`](./src/executor/), `WEAVER_EXECUTOR`, default local SDK `query()`); supervision and submit callbacks stay harness-owned. Inherited MCP config and its referenced environment cross as one capability: every executor provisions all of it or fails before the loop, never silently drops servers. Ambient process env and Claude account credentials are substrate-local and never cross the seam.
