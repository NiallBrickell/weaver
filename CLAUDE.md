...
- **Never put a personal GitHub login or PAT on a hosted runner.** Hosted repository access uses a dedicated GitHub App installed only on managed repositories; its short-lived tokens are narrowed to the exact repository and read/write lifecycle. Ordinary containers get neither the App key nor a token, same-UID model actions are refused, and only an exact approved engine command receives write capability. See [docs/harness.md](./docs/harness.md).
